...
def assert_fee_amount(fee, tx_size, fee_per_kB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s CUA too low! (Should be %s CUA)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    if fee > (tx_size + 2) * fee_per_kB / 1000:
        raise AssertionError("Fee of %s CUA too high! (Should be %s CUA)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting CUA values"""
