...
RCSUBDIR="test"
HOST1="https://coinuacore.net"
HOST2="https://coinuacore.net"
BASEDIR="/bin/"
...
if ! WGETOUT=$(wget -N -O "$SIGNATUREFILENAME.2" "$HOST2$BASEDIR$SIGNATUREFILENAME" 2>&1); then
   echo "coinuacore.net failed to provide signature file, but bitcoincore.org did?"
   echo "wget output:"
   # shellcheck disable=SC2001
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "coinuacore.net and coinuacore.net signature files were not equal?"
