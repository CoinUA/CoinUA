...
class AvoidReuseTest(BitcoinTestFramework):
    ...
    def test_sending_from_reused_address_without_avoid_reuse(self):
        '''
        Test the same as test_sending_from_reused_address_fails, except send the 10 CUA with
        the avoid_reuse flag set to false. This means the 10 CUA send should succeed,
    ...
    def test_sending_from_reused_address_fails(self, second_addr_type):
        ...
        [0] sends 10 CUA to A.
        [1] spends 5 CUA from A. (leaving roughly 5 CUA useable)
        [0] sends 10 CUA to A again.
        [1] tries to spend 10 CUA (fails; dirty).
        [1] tries to spend 4 CUA (succeeds; change address sufficient)
        '''
    ...
    def test_full_destination_group_is_preferred(self):
        '''
        Test the case where [1] only has 11 outputs of 1 CUA in the same reused
        address and tries to send a small payment of 0.5 CUA. The wallet
        should use 10 outputs from the reused address as inputs and not a
        single 1 CUA input, in order to join several outputs from the reused
        address.
        '''
        ...
        # Send 11 outputs of 1 CUA to the same, reused address in the wallet
        for _ in range(11):
            self.nodes[0].sendtoaddress(new_addr, 1)
    ...
    def test_all_destination_groups_are_used(self):
        '''
        Test the case where [1] only has 22 outputs of 1 CUA in the same reused
        address and tries to send a payment of 20.5 CUA. The wallet
        ...
        # Send 22 outputs of 1 CUA to the same, reused address in the wallet
        for _ in range(22):
            self.nodes[0].sendtoaddress(new_addr, 1)
