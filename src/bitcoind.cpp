...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.IsArgSet("-version")) {
        ...
        if (args.IsArgSet("-version")) {
            ...
            strUsage += "\nUsage:  coinuad [options]                     Start " PACKAGE_NAME "\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see coinuad -h for a list of options.\n", argv[i])));
            }
        }
